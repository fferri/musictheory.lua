...
function Note.static:parse(s)
    if type(s) == 'string' then
        -- Simple pattern matching: last digit is octave, rest is pitch class
        local octave = tonumber(s:sub(-1))
        local pc_str = s:sub(1, -2)
        local pc = PitchClass:parse(pc_str)
...
function Interval:__add(other)
    if other.class == Interval then
        local Note = Note -- Assume Note class exists
        local n = Note('C0')
...
function Chord:recipe()
    ...
    local function identify(intervals)
        local n = #intervals
        local masks = {}
        for mask = 0, (1 << n) - 1 do
        ...
        for _, mask in ipairs(masks) do
            ...
            for i, interval in ipairs(intervals) do
                if mask & (1 << (i-1)) ~= 0 then
                    if interval:is_compound() or interval == Interval:parse('P8') then
                        table.insert(new_intervals, interval:reduce())
                    else
                        table.insert(new_intervals, interval + Interval:parse('P8'))
                    end
                else
            ...
            if Chord.recipes_inv[key] then
                local recipe_name = Chord.recipes_inv[key]
...
function Chord.static:identify_from_notes(notes)
    ...
    for _, mask in ipairs(masks) do
        ...
        local base_note = new_notes[1]
        local intervals = {}
        for _, note in ipairs(new_notes) do
            table.insert(intervals, note - base_note)
        end
    ...
    describe('TestsForNote', function()
        ...
        it('note_from_number', function()
            local function test1(num, maxacc, notes)
                local ns = {}
                for _, n in ipairs(notes) do ns[Note(n)] = true end
                local allacc
                ...
                local notes_set = Note.from_number(num, {allowed_accidentals = allacc})
                for n in pairs(notes_set) do
                    assert.is_true(ns[n])
                end
            end
            test1(59, -1, {'B4'})
